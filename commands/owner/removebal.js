...
module.exports = {
    name: 'removebal',
    description: 'Trừ bớt Lux của người dùng (Chỉ Owner)',
    slashData: new SlashCommandBuilder()
        .setName('removebal')
        .setDescription('Trừ bớt Lux của người dùng (Chỉ Owner)')
        .addStringOption(opt => opt.setName('target').setDescription('Ping hoặc User ID').setRequired(true))
        .addIntegerOption(opt => opt.setName('amount').setDescription('Số tiền Lux cần trừ').setRequired(true)),
    ...
    async execute(context, args, isSlash) {
        ...
        if (!targetInput || isNaN(amount) || amount <= 0) {
            const usageMsg = '⚠️ Cú pháp: `l!removebal <@user|id> <amount>` (Số tiền > 0)';
        ...
        db.users[targetUser.id].balance = Math.max(0, db.users[targetUser.id].balance - amount);
        context.client.writeDB(db);

        const embed = new EmbedBuilder()
            .setColor('#FF0000')
            .setTitle('👑 Bot Owner - Trừ Số Dư')
            .setDescription(`Đã trừ **${amount.toLocaleString()} Lux ($LX$)** của **${targetUser.username}**.\n💰 Số dư mới: **${db.users[targetUser.id].balance.toLocaleString()} Lux**`);
